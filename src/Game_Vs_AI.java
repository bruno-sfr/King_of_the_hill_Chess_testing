...
public class Game_Vs_AI {
    ...
    public ReturnObject minimax(int depthleft, boolean player, Chessboard board, ReturnObject object){
        if(depthleft==0){
            //object.eval=board.eval_func();
            //System.out.println("depth==0: returning: "+returnvalue[0]+","+returnvalue[1]+","+returnvalue[2]);
            return new ReturnObject(board.eval_func(),object.moves);
        ...
        if(player){
            ...
            for(LinkedList<ChessMove> moves:allMoves){
                for(ChessMove move:moves){
                    ...
                    ReturnObject eval=minimax(depthleft-1,false,child,new ReturnObject(object.eval,Movelist));
            ...
            for(LinkedList<ChessMove> moves:allMoves){
                for(ChessMove move:moves){
                    ...
                    LinkedList<ChessMove> Movelist=(LinkedList<ChessMove>) object.moves.clone();
                    Movelist.add(move);
                    ReturnObject eval=minimax(depthleft-1,true,child,new ReturnObject(object.eval,Movelist));
